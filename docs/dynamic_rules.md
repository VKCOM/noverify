...
You can see examples of dynamic rules used in NoVerify at [link](/src/cmd/embeddedrules/rules.php).
...
Let's search for Yoda style comparisons, for example:
...
/**
 * @comment Report function calls for which there are more commonly used aliases.
 * @before  sizeof($x)
 * @after   count($x)
 */
function funcAlias() {
...
The example above contains two groups of rules, `funcAlias` and `strictCmp`.
...
- `@comment` — description of the checks in the group, used in the documentation of the checks;
...
Each rule is a phpgrep template. To understand the patterns, read the description of the phpgrep format [here](https://github.com/quasilyte/phpgrep/blob/master/pattern_language.md).
...
Each rule is an expression or statement ([phpdoc](https://github.com/quasilyte/phpgrep/blob/master/pattern_language.md) pattern) over which PHPDoc is written with additional meta information. Each rule should have a definition of the severity level and a message if the rule was matched.
...
An example of a minimum rule with a statement:
...
For every warning that NoVerify finds, it underlines the location. However, for dynamic rules, the right place is not always emphasized.
...
The linter is currently underlining `for`, but the reason for reporting is in the `count` function, and we would like to emphasize it.
