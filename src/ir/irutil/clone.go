...
func NodeClone(x ir.Node) ir.Node {
	...
	case *ir.Argument:
		clone := *x
		if x.ArgName != nil {
			clone.ArgName = NodeClone(x.ArgName).(*ir.Identifier)
		}
		if x.Expr != nil {
	...
	case *ir.MatchArm:
		...
		clone.Exprs = NodeSliceClone(x.Exprs)
		if x.ReturnExpr != nil {
			clone.ReturnExpr = NodeClone(x.ReturnExpr)
		}
		return &clone
	case *ir.MatchExpr:
		clone := *x
		...
		{
			sliceClone := make([]*ir.MatchArm, len(x.Arms))
			for i := range x.Arms {
				sliceClone[i] = NodeClone(x.Arms[i]).(*ir.MatchArm)
			}
			clone.Arms = sliceClone
		}
		return &clone
