...
func gitRepoComputeReportsFromCommits(l *linterRunner, logArgs, diffArgs []string) (oldReports, reports []*linter.Report, changes []git.Change, changeLog []git.Commit, ok bool) {
	...
	if l.args.gitFullDiff {
		...
		start := time.Now()
		linter.ParseFilenames(workspace.ReadFilesFromGit(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitFrom, nil), l.allowDisableRegex)
		...
		start = time.Now()
		oldReports = linter.ParseFilenames(workspace.ReadFilesFromGit(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitFrom, linter.ExcludeRegex), l.allowDisableRegex)
		...
		start = time.Now()
		parseIndexOnlyFiles(l)
		linter.ParseFilenames(workspace.ReadFilesFromGit(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitTo, nil), l.allowDisableRegex)
		log.Printf("Indexed new commit in %s", time.Since(start))

		meta.SetIndexingComplete(true)

		start = time.Now()
		reports = linter.ParseFilenames(workspace.ReadFilesFromGit(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitTo, linter.ExcludeRegex), l.allowDisableRegex)
		...
		start = time.Now()
		oldReports = linter.ParseFilenames(workspace.ReadOldFilesFromGit(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitFrom, changes), l.allowDisableRegex)
		log.Printf("Parsed old files versions for %s", time.Since(start))

		start = time.Now()
		meta.SetIndexingComplete(false)
		parseIndexOnlyFiles(l)
		linter.ParseFilenames(workspace.ReadFilesFromGitWithChanges(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitTo, changes), l.allowDisableRegex)
		meta.SetIndexingComplete(true)
		log.Printf("Indexed files versions for %s", time.Since(start))

		start = time.Now()
		reports = linter.ParseFilenames(workspace.ReadFilesFromGitWithChanges(linter.GitDir, l.args.gitRepo, l.args.mutable.gitCommitTo, changes), l.allowDisableRegex)
