...
func gitMain() (int, error) {
	...
	if !ok {
		...
		if !ok {
			log.Printf("No changes, no reports to compute")
			return 0, nil
		}
		log.Printf("Used commited changes to compute reports")
	} else {
		log.Printf("Used local changes to compute reports")
	}

	fmt.Println("OLD:")
	fmt.Println(oldReports)
	fmt.Println("NEW:")
	fmt.Println(reports)
