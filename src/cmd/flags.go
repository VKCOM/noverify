...
func bindFlags(ruleSets []*rules.Set, args *cmdlineArguments) {
	...
	flag.IntVar(&config.MaxFileSize, "max-sum-filesize", 20*1024*1024, "max total file size to be parsed concurrently in bytes (limits max memory consumption)")
	flag.IntVar(&config.MaxConcurrency, "cores", runtime.NumCPU(), "max cores")
	flag.BoolVar(&config.LangServer, "lang-server", false, "Run language server for VS Code")

	flag.StringVar(&config.StubsDir, "stubs-dir", "", "phpstorm-stubs directory")
