...
func NewMap(str string) Map {
	...
	for _, typ := range strings.Split(str, "|") {
		if IsArray(typ) {
			typ = WrapArrayOf(ArrayType(typ))
		}
		m[typ] = struct{}{}
	}
	return Map{m: m}
...
// Append adds provided types to current map and returns new one
//
// Note: immutable maps are always copied
...
// LazyArrayElemType returns type of array element.
//
// T[] -> T, T[][] -> T[].
