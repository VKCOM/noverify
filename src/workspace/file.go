...
func (f *File) ParseStartPos(pos *position.Position) (startLn []byte, startChar int) {
	if pos.StartLine >= 1 && f.NumLines() > pos.StartLine {
		startLn = f.Line(pos.StartLine - 1)
		p := f.LinePosition(pos.StartLine - 1)
		if pos.StartPos > p {
...
func (f *File) NodeText(n ir.Node) string {
