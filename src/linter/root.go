...
func (d *RootWalker) InitFromParser(contents []byte, parser *php7.Parser) {
	...
	d.Lines = lines
	d.autoGenerated = workspace.FileIsAutoGenerated(contents)
}
...
func (d *RootWalker) report(n ir.Node, lineNumber int, level int, checkName, msg string, args ...interface{}) {
	...
	if d.autoGenerated && !config.CheckAutoGenerated {
		return
	}
	...
	if isReportForNode {
		if n == nil {
			...
			if strings.Contains(msg, "syntax error") && strings.Contains(msg, " at line ") && !config.LangServer {
...
func (d *RootWalker) reportHash(pos *position.Position, startLine []byte, checkName, msg string) uint64 {
	...
	if !config.ConservativeBaseline {
...
func (d *RootWalker) handleFuncStmts(params []meta.FuncParam, uses, stmts []ir.Node, sc *meta.Scope) handleFuncResult {
	...
	for _, p := range params {
		...
		if !p.IsRef && !config.IsDiscardVar(p.Name) {
...
func (d *RootWalker) checkMisspellings(n ir.Node, s string, label string, skip func(string) bool) {
	...
	if config.TypoFixer == nil {
		return
	}
	_, changes := config.TypoFixer.Replace(s)
...
func (d *RootWalker) runRule(n ir.Node, sc *meta.Scope, rule *rules.Rule) bool {
	...
	if config.ApplyQuickFixes && rule.Fix != "" {
